...
const blankExpense = () => ({ category_id: categories.value[0]?.id || '', provider_id: '', amount: 2500, paid_via: 'pix', expense_date: new Date().toLocaleDateString('sv-SE'), note: '', recurring: false });
const exForm = ref({ category_id: '', provider_id: '', amount: 2500, paid_via: 'pix', expense_date: new Date().toLocaleDateString('sv-SE'), note: '', recurring: false });
...
const trendSeries = computed(() => [
  { label: 'Receita ganha', color: '#0d9488', values: (trend.value || []).map((p) => Math.round(p.earnedCents / 100)) },
  { label: 'Despesas', color: '#d97706', values: (trend.value || []).map((p) => Math.round(p.spentCents / 100)) },
]);
...
// System billing: our own LavTr subscription invoices (read-only, loaded
...
const billingColumns = [
  ...
  { key: 'paid', label: 'Pago', align: 'right' },
...
async function exportCsv() {
  ...
  const rows = [
    ['LavTr P&L', monthLabel(month.value)],
  ...
  <div>
    ...
    <Panel v-if="activeTab === 'expenses'"
      ...
      <DataTable :columns="exColumns" :page="exPage" empty-text="No money spent in this period." @page="loadExpenses">
        ...
        <template #cell-paidVia="{ row }">{{ row.paidVia === 'pix' ? 'Pix' : row.paidVia === 'pix' ? 'M-Pesa' : 'Dinheiro' }}</template>
    ...
    <Panel v-if="activeTab === 'providers'" title="Fornecedores"
    ...
    <Modal v-if="payModal" title="Record a payment" @close="payModal = false">
      ...
      <FormField v-if="payForm.method === 'pix_manual'" label="Código da transação Pix">
    ...
    <Panel v-if="activeTab === 'billing'" title="Sua fatura LavTr"
      subtitle="Invoices for your LavTr subscription — what you've been billed, what you've paid, and what's still to pay. Paid invoices are added to your expenses automatically.">
      <template #actions>
    ...
    <Modal v-if="billingDetail" :title="`Invoice ${billingDetail.invoice.number}`" wide @close="billingDetail = null">
      ...
      <div v-if="!billingDetail.payments.length" class="muted small">Nenhum pagamento registrado ainda. Pague via Pix — a equipe LavTr registra o valor nesta fatura.</div>
